...
(function(){
  ...
  //SELECT
  $ = function(a, b, c){						//goal, nocache, limit node
    if(typeof(a) == nil){
      return;
    }
  ...
  $.E($, {
    ...
    post: function(url, data, post_callback, n){
      n                    = n || 1
      $._script_channel[n] = new $._setChannel(url, '', '', post_callback);
      $.D(data, n);
    }
    ,
    FUN: function(a){
            ...
            if(r >= 0){
              if(z.fn){
                ...
                for(var i in
                  z.from){
                  if(i == 'BG'){
                    y[i] = $.MIXCOLOR(r, z.from[i], z.to[i]);
                  }
                  else{
                    y[i] = z.twfunc(r, z.from[i], z.dis[i]);
                  }
                }
                z.mylord.S(y);
              }
    ...
    C: function(father, css, tag){	//father, tag 也行
      ...
      if(tag == 'password'){
        ...
        var z = $(typeof(tag) == 'string' ? d.createElement(tag) : tag);	//document.createElement
      }
    ...
    _bindFunc: function(evt){
      return function(fun, bubble, fn2){
        this.bind(evt, fun, bubble, fn2);
        return this;
      }
    }
    ...
    QS: function(key){
      ...
      if(z > 0){
        ...
        //遍历分解后的键值对
        for(var i = 0, len = arrayURLParams.length;
          i < len;
          i++){
          //分解一个键值对
    ...
    MIXCOLOR: function(p, c1, c2){
      var
        r1   = '0x' + c1.substring(1, 3)
        , g1 = '0x' + c1.substring(3, 5)
        , b1 = '0x' + c1.substring(5, 7)
        , r2 = '0x' + c2.substring(1, 3)
        , g2 = '0x' + c2.substring(3, 5)
        , b2 = '0x' + c2.substring(5, 7)
        , r  = Math.round(r2 * p + r1 * (1 - p)).toString(16)
        , g  = Math.round(g2 * p + g1 * (1 - p)).toString(16)
        , b  = Math.round(b2 * p + b1 * (1 - p)).toString(16);
  ...
  $._init.prototype = {
    ...
    S: function(css, done){
      ...
      var n = t.context, z = n.style, y, x, w, I;
      for(var i in
        css){
        y = css[i];
    ...
    I: function(a){
      if(this.CHILDREN){
        for(var i = this.CHILDREN.length - 1;
          i >= 0;
          i--){
          this.CHILDREN[i].R();
        }
    ...
    bind: function(type, fn, bubble, fn2){
      ...
      var t     = this
        , _ctx  = t.context
        , type_ = type.toUpperCase()	//t.MOUSEDOWN 存放着down事件
        , fun;
            ...
            switch(type){
              case 'mousedown':
                ...
                if(t.MOUSEMOVE){
                  ...
                  if(t.MOUSEUP && t.is_drag && !t.catch_mouse){
                    var w         = 500;
                    t.catch_mouse = $.C(t, {
                      L: -w
                      ,
              ...
              case 'mouseup':
                ...
                if(fn2
                  && $.MS() - t.down_time < 200
                  && Math.abs(t.X - t.DOWN_X) < 3
                  && Math.abs(t.Y - t.DOWN_Y) < 3
                ){
                  //有效点击
                  fn2.call(_ctx, t);
                }
    ...
    DRAG: function(option){
      option = option || {};		//这样，访问option的属性不会出错了
      if(!('x' in option)){
        option.x = [];
      }
      if(!('y' in option)){
      ...
      var z = this;
      var G = z.goal = option.goal || z;

      if(option.x && G.FATHER.W_){
      ...
      z.mousedown(function(z){
          var G = z.goal;
          if(G.Z_ != $.Z && !option.zkeep){
            G.S({Z: $.Z++});
          }
        }, 0, option.mousedown)	//曾经设定为冒泡，不知为何 7-16
        .mouseup(option.mouseup || function(z){
          }, 0, option.click)
    ...
    hover: function(out, over){
      this
        ...
        .mouseover(typeof(over) == 'function'
          ? over
          : function(z){
  ...
  $.IS.T    = 'createTouch' in document || 'ontouchstart' in window
  //$.IS.MH || $.IS.MP || $.IS.A;
  ...
  //方法REPEATER，需要用到参数2
  $.E($, {
    REPEATER: new function(){
      var z = $._pre.toLowerCase() + 'RequestAnimationFrame';
      var y = 'requestAnimationFrame';
      return window[y]
        ? function(fn){
        window[y](fn)
      }
  ...
  $.timer = function(){
    if(d.body && $._funlist && $._img_ok && chartsetok){
      d.body.onload && $._funlist.push(d.body.onload);
      for(var i = 0;
        i < $._funlist.length;
        i++){	//当$._funlist[i](window)运行时，$._funlist数组还可能添加
