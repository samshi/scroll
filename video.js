...
var media_controller_properties = [
  "readyState", "buffered", "seekable", "duration", "currentTime",
...
function capture(event){
  ...
  //console.log('media_events', event.type, media_events[event.type]);
}

function mc_capture(event){
  media_controller_events[event.type]++;
  //console.log('media_controller_events', event.type, media_controller_events[event.type]);
...
function update_properties(){
  ...
  for(key in media_controller_events){
    ...
    if(e){
      ...
      if(media_controller_events[key] > 0){
        e.className = "true";
      }
    }
  }
  for(key in media_properties){
    ...
    if(media_properties[key]=='currentTime'){
      console.log(media_properties[key], val);
    }
  }
  if(document._hasController){
    i = 0;
